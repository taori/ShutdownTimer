...
namespace ShutdownTimer.Server.WindowsService.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
            ...
            if (ModelState.IsValid)
            {
	            using (var transactionScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
				{
					...
					if (result.Succeeded)
					{
						...
						{
							var addAdmin = await _userManager.AddToRoleAsync(user, WellKnownRoleNames.Administrator);
							if (!addAdmin.Succeeded)
							{
								_logger.LogError($"Failed to add user {user.NormalizedUserName} to role {WellKnownRoleNames.Administrator}.");
